...
class Minitaur(object):
    ...
    def SetMotorGains(self, kp, kd):
        ...
        """
        if isinstance(kp, (Sequence, np.ndarray)):
            self._motor_kps = np.asarray(kp)
        else:
            self._motor_kps = np.full(self.num_motors, kp)

        if isinstance(kd, (Sequence, np.ndarray)):
